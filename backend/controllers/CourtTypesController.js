...
export const getCourtTypeByTenantIdAndVenueController = async (req, res) => {
  try {
    ...
    if (response.length === 0) {
      return res
        .status(404)
        .json({ message: "No court types found for the given tenant and venue" });
    }
...
export const createCourtTypeDetailsController = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: "Failed to create court type details. Please try again later." });
  }
};
